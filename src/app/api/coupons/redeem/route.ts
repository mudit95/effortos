...
export async function POST(req: Request) {
  ...
  // ── Self-referral guard ──────────────────────────────────────────
  // Reject before we even touch the redemption RPC if this code was
  // generated by the calling user. Cheaper than rolling back and gives
  // a clearer error message.
  {
    ...
    const { data: ownerCheck } = await service
      ...
      .maybeSingle();
    if (ownerCheck?.referrer_user_id && ownerCheck.referrer_user_id === user.id) {
  ...
  // ── Referral kickback ────────────────────────────────────────────
  // If this coupon was generated by an end user (referrer_user_id set),
  // credit them with the configured kickback months. We do this AFTER
  // the redeemer's reward lands so a kickback failure doesn't block
  // the friend's redemption — the friend already got value, the
  // referrer credit is best-effort. Failure is logged loudly so ops
  // can spot the rare drift and refund manually.
  //
  // Only applies to free_months redemptions today. Adding kickbacks for
  // trial_extension / percent_off would require a different reward
  // shape on the referrer side.
  if (r.kind === 'free_months') {
    const { data: couponRow } = await service
      .from('coupons')
      .select('referrer_user_id, referrer_kickback_months')
      .eq('id', r.coupon_id)
      .maybeSingle();

    if (couponRow?.referrer_user_id && couponRow.referrer_kickback_months) {
      const kickbackMonths = Number(couponRow.referrer_kickback_months);
      const { error: kickbackErr } = await service.rpc('credit_referrer_kickback', {
        p_user_id: couponRow.referrer_user_id,
        p_months: kickbackMonths,
      });
      if (kickbackErr) {
        console.error('[coupons/redeem] kickback failed AFTER redeemer was rewarded:', {
          referrer_user_id: couponRow.referrer_user_id,
          coupon_id: r.coupon_id,
          months: kickbackMonths,
